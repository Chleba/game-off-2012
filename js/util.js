...
BattleMage.cUtil = JAK.ClassMaker.makeClass({
	NAME : 'BattleMage.cUtil',
	VERSION : '1.0'
});

BattleMage.cUtil.prototype.$constructor = function(canvas, opt){
	this.canvas = canvas;
	this.color = '#666';
	this.Lopt = {
	    lwidth : 1,
...
BattleMage.cUtil.prototype.randRange = function(min, max){
	var rand = (Math.floor(Math.random() * (min-max+1))+min)*-1;
	//var rand = Math.floor(Math.random() * (max - min + 1) + min)
	return rand;
};

BattleMage.cUtil.prototype.makeLine = function(f, t){
	this.canvas.save();
...
BattleMage.cUtil.prototype.makeLight = function(f, t){
	this.canvas.save();
	this.canvas.beginPath();
...
BattleMage.cUtil.prototype.makeCircleLight = function(f, r){
	var a = 0;
	var tx = 0;
	var ty = 0;
	do{
