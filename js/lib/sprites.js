...
Sprite.prototype.$constructor = function(canvas, opt){
	...
	this.animation = this.opt.animation || (this.jsonMap.spriteBuddy.animations.animation instanceof Array ? this.jsonMap.spriteBuddy.animations.animation[0] : this.jsonMap.spriteBuddy.animations.animation);
	this._make()
};

Sprite.prototype.changeAnimation = function(animation){
...
Sprite.prototype.drawSingleFrame = function(pos, frame){
	var layer = frame.layers.layer;
	this.draw(pos, layer);
};

Sprite.prototype.draw = function(pos, layer){
	var pos = pos || { x : 0, y : 0 };
	...
	var layer = layer || this.frames[index].layers.layer;
